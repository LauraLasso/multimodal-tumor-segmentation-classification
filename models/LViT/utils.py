...
def read_text(filename):
    ...
    for i in df.index.values:  # Gets the index of the row number and traverses it
        ...
        if count < 9:
            df.description[i] = df.description[i] + ' EOF XXX' * (9 - count)
        text[df.filename[i]] = df.description[i]
    return text  # return dict (key: values)

def read_text_brats2020(filename):
    df = pd.read_excel(filename)
    info_dict = {}
    ...
    for i in df.index.values:
        ...
        if count < 9:
            description = description + ' EOF XXX' * (9 - count)

        filename = df.filename[i]
        age = df.Age[i]
        survival = df.Survival_days[i]
    ...
    return info_dict  # dict con toda la info
