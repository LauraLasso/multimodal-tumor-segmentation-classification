...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        image, label, text, age, target = sample['image'], sample['label'], sample['text'], sample['age'], sample['target']
        ...
        sample = {'image': image, 'label': label, 'text': text,  'age':age, 'target':target}
        return sample
...
# Esta clase devuelve un diccionario cuando se crea el dataloader con eso
# row_text es la descripción de la segmentación
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image_filename = self.images_list[idx] # BCBM
        # mask_filename = image_filename.replace('_img.png', '_mask.png')
        ...
        mask_filename = self.mask_list[idx]  # BraTS2020
        image_filename = mask_filename.replace('_seg', '')
        ...
        # Descripción con la edad y SD
        info = self.rowtext[mask_filename]

        text = info['text'].split('\n')
        age = np.array([info['age']], dtype=np.float32)
        target = np.array([info['survival_days']], dtype=np.float32)
        ...
        sample = {'image': image, 'label': mask, 'text': text, 'age': age, 'target': target}
        # print('sample de load_dataset')
        # print(sample)
