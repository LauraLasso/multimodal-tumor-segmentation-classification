...
from utils import CosineAnnealingWarmRestarts, WeightedDiceBCE, WeightedDiceCE, read_text, read_text_LV, save_on_batch, read_text_brats2020
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    elif config.task_name == 'Covid19 CT':
        train_text = read_text(config.train_dataset + 'Train_text.xlsx')
    ...
    elif config.task_name == 'BCBM-RadioGenomics':
        train_text = read_text(config.train_dataset + 'Train_text.xlsx')
        # print(config.train_dataset)
        # print(config.val_dataset)
        # print(config.task_name)
    ...
    if model_type == 'LViT':
        ...
        logger.info('transformer expand ratio: {}'.format(config_vit.expand_ratio))
        model = LViT(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
        # C:\Users\Laura\Documentos\Redes multimodales\LViT\MoNuSeg\LViT\Test_session_03.25_11h44\models\best_model-LViT.pth.tar
        pretrained_UNet_model_path = "MoNuSeg/LViT/Test_session_03.25_11h44/models/best_model-LViT.pth.tar"
