...
import os
from models.sam2.medsam2.utils import *
from tqdm import tqdm


def train_sam(net: nn.Module, optimizer, train_loader, epoch, writer, criterion_G, mask_type, GPUdevice, out_size, image_size, memory_bank_size):
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}', unit='img') as pbar:
        for ind, pack in enumerate(train_loader):
            ...
            high_res_multimasks = F.interpolate(low_res_multimasks, size=(image_size, image_size),
...
def validation_sam(val_loader, epoch, net: nn.Module, criterion_G, writer, out_size, image_size, path_helper, clean_dir=True):
    ...
    GPUdevice = torch.device('cuda:' + str(gpu_device))
        ...
        for ind, pack in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                if ind % vis == 0:
                    ...
                    vis_image(imgs,pred, masks, os.path.join(path_helper['sample_path'], namecat+'epoch+' +str(epoch) + '.jpg'), reverse=False, points=None)

            pbar.update()

            writer.add_scalar("Val/Loss", total_loss / n_val, epoch)
            writer.add_scalar("Val/IOU", total_eiou / n_val, epoch)
            writer.add_scalar("Val/DICE", total_dice / n_val, epoch)
